    ...
    if connect()
      ...
      if ldap.bind
        ...
        if results.present?
          ...
          puts("Autenticacion no satisfactoria, el usuario no se encuentra registrado en la base de datos.")
          @newAuth = ObjAuth.new(email, password, "false")
          render json: @newAuth
        end
      else
        puts("Autenticacion no satisfactoria, el usuario no se encuentra registrado en el LDAP.")
        @newAuth = ObjAuth.new(email, password, "false")
        render json: @newAuth
      end
    end
